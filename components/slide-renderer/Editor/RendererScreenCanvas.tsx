...
import { Film, ImageOff, Paintbrush, RotateCcw, ShieldAlert, VideoOff, Fullscreen } from 'lucide-react';
...
function PlaybackVideoContent({
  ...
  if (resolvedSrc) {
    return (
      <div ref={scope} className="relative h-full w-full group/video">
        <video
          ref={videoRef}
        ...
        <button
          ...
          onPointerDown={(e) => e.stopPropagation()}
          className="absolute right-2 bottom-2 flex items-center gap-1 rounded bg-black/60 px-2 py-1 text-white opacity-0 group-hover/video:opacity-100 transition-opacity duration-200 hover:bg-black/80"
          title={t('stage.fullscreen')}
        >
          <Fullscreen className="h-3 w-3" />
          <span className="text-[10px] font-medium">{t('stage.fullscreen')}</span>
        </button>
        {canRetry ? (
          <button
            onClick={(event) => {
